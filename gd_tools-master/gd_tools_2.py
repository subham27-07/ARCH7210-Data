class Node:
	...
	def get_adjacent(self):
		'''This function gets the adjacent edges'''
		adjacent = []
		edges = []
	...
	def reset(self):
		'''reset the distance between nodes'''
		self.distance = float("inf")
		self.visited = False 
		self.previous = None 
		self.previous_edge = None 
...
class Graph:
	''' Drawing the graph between nodes'''
	def __init__(self):
		self.nodes = []
		self.edges = []
		self.dest_node = None
	...
	def find_closest_node(self, p, err=0.001):
		...
		min_node = None
		for node in self.nodes:
			# check the condition if the node is lower than minimum distance
	...
	def add_edge(self, line, err, cost=None):
		edge_id = len(self.get_edges())
		#create an empty list of node to be added
		nodes = []
		for i, p in enumerate(line):
			nodes.append(self.find_node(p, err))
			if nodes[-1] is None:
				nodes[-1] = self.add_node(p)
		#define the new edge to be formed
...
# DIJKSTRA shortest path implemention :
...
def graph_from_lines_test():
	...
	lines_data =[[1.0, 0.0, 0.0], [2.0, 1.0, 0.0]], 
	
				

	graph = lines2graph(lines_data)

	sources = [[35.2252808, -80.8409319, 0]]
	destinations = [[35.3121753, -80.7337248, 0]]
